...
export const Scene = () => {
  ...
  const redRayBoxRef = useRef();
  const blueRayBoxRef = useRef();
  ...
  const handleFishClick = () => {
    if (gameState.currentGame === 1) {
      setGameState((prev) => ({
        ...
        showPopup: true,
        popupMessage:
          "Ce n'est pas le bon objet ! Un poisson devrait être dans l'eau, pas dans le ciel. Essayez encore !",
      }));
    }
  };
  ...
  const handleCarClick = () => {
    if (gameState.currentGame === 1) {
      setGameState((prev) => ({
        ...
        showPopup: true,
        popupMessage:
          "Ce n'est pas le bon objet ! Une voiture devrait être sur la route, pas dans le ciel. Essayez encore !",
      }));
    }
  };
  ...
  const handleWrongRayClick = () => {
    if (gameState.currentGame === 2 && !gameState.game2Completed) {
      setGameState((prev) => ({
        ...prev,
        showPopup: true,
        popupMessage: "Ce n'est pas le bon rayon !",
      }));
    }
  };
  ...
  return (
    <>
      ...
      <group ref={planeRef} position={[50, 50, 0]} rotation={[0, 5, 0]}>
        ...
        <SceneObject modelPath="/assets/models/ciel/plane.glb" scale={1} />
      </group>
      ...
      <group position={[baseX, rayHeight, baseZ]}>
        <mesh
          ...
          onPointerOut={() => (document.body.style.cursor = "auto")}
        >
          <cylinderGeometry args={[15, 15, 200, 16]} />
          <meshBasicMaterial opacity={0} transparent />
        </mesh>
        <SunRay
          startPosition={[0, 0, 0]}
      ...
      <group position={[baseX, rayHeight, baseZ + spacing]}>
        <mesh
          ref={redRayBoxRef}
          visible={false}
          onClick={handleWrongRayClick}
          onPointerOver={() =>
            gameState.currentGame === 2 &&
            (document.body.style.cursor = "pointer")
          }
      ...
      <group position={[baseX, rayHeight, baseZ + spacing * 2]}>
        <mesh
          ref={blueRayBoxRef}
          visible={false}
          onClick={handleWrongRayClick}
        ...
        {gameState.showPathGame && (
          <div
            style={{
              ...
              borderRadius: "10px",
              zIndex: 900,
              display: gameState.game3Completed ? "none" : "block",
            }}
          >
            <canvas
              ref={pathCanvasRef}
